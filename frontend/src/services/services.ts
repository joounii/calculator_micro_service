...
export class BasicOpsService {
    ...
    calc(a: number, b: number, op: BasicOp) {
        const body: BasicCalcBody = { a: Number(a), b: Number(b), op };
        return this.client.request<BasicCalcResp, BasicCalcBody>("/ops/basic", { method: "POST", body });
    }
}

export class AdvancedMathService {
    constructor(private client: ApiClient) {}
    run(fn: string, args: number[]) {
        const body: AdvancedRunBody = { fn, args };
        return this.client.request<AdvancedRunResp, AdvancedRunBody>("/ops/advanced", { method: "POST", body });
    }
}
...
export class HistoryService {
    ...
    list(params?: Record<string, string | number | boolean>) {
        ...
        return this.client.request<HistoryListResp>(`/history${qs}`);
    }
    add(entry: HistoryAddBody) {
        return this.client.request<HistoryAddResp, HistoryAddBody>("/history", { method: "POST", body: entry });
    }
}
...
export class UserService {
    constructor(private client: ApiClient) {}
    me() { return this.client.request<MeResp>("/me"); }

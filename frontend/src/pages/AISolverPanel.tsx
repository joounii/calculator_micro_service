...
import type { AiSolveResp } from "@/services/services";

export const AISolverPanel: React.FC = () => {
    const { services } = useServices();
    const [prompt, setPrompt] = useState<string>(
        "Ein Auto beschleunigt von 0 auf 100 km/h in 8 s. Wie gross ist die mittlere Beschleunigung?"
    );
    const [verify, setVerify] = useState<boolean>(true);
    const [units, setUnits] = useState<string>("SI");
    const [resp, setResp] = useState<AiSolveResp | null>(null);
    const [loading, setLoading] = useState<boolean>(false);

    async function solve() {
        setLoading(true);
        try {
            const data: AiSolveResp = await services.ai.solve(prompt, { units }, verify);
            setResp(data);
            await services.history.add({ kind: "ai", input: { prompt }, result: data.result });
        } catch (e: unknown) {
            ...
            <Card variant="outlined">
                ...
                <CardContent>
                    ...
                    <Grid container spacing={2} alignItems="center">
                        <Grid size={{ xs: 12, sm: 6 }}>
                            <FormControl fullWidth>
                                ...
                                <Select
                                    label="Einheiten"
                                    value={units}
                                    onChange={e => setUnits(e.target.value as string)}
                                >
                        ...
                        <Grid size={{ xs: 12, sm: 6 }}>
                            <FormControlLabel
                                control={<Switch checked={verify} onChange={e => setVerify(e.target.checked)} />}
                                label="Verifikation aktiv"
                            />
                        </Grid>
                        <Grid size={12}>
                            <Button
                        ...
                        <Box mt={3}>
                            ...
                            <Stack direction="row" spacing={2} alignItems="center" mt={1}>
                                {typeof resp.result === "number" && (
                                    <Chip label={`Ergebnis: ${formatNumber(resp.result)}`} color="primary" />
                                )}
